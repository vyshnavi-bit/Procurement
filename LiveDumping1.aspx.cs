...
public partial class LiveDumping1 : System.Web.UI.Page
{
...

    {
        if (IsPostBack != true)
        {
            ...
            {
              //  int userid = Convert.ToUInt16(Session["User_ID"]);
...
" (SELECT MIN(StartingTime) AS Stime,MAX(EndingTime) AS Etime,PlantCode  FROM Lp Where   prdate='" + date + "' and  sessions='" + sess + "' GROUP BY PlantCode) AS t2 ON t1.PlantCode=t2.PlantCode ";
    ...
    private void getdetails()
    {
        try
        {
            ...
            using (conn = dbaccess.GetConnection())
            {
                ...
                SqlCommand sqlCmd = new SqlCommand("[Lp_procurementShowAll1]");
    ...
    private void getconnectionr()
    {
        try
        {
            ...
            using (conn = dbaccess.GetConnection())
            {
                SqlCommand sqlCmd = new SqlCommand("dbo.Lp_procurementConncetionShow1");
                sqlCmd.Connection = conn;
                sqlCmd.CommandType = CommandType.StoredProcedure;
                sqlCmd.Parameters.Add(new SqlParameter("@fdate", date));
                sqlCmd.Parameters.Add(new SqlParameter("@session", sess));
    ...
    private DataTable GetData()
    {
        ...
        DataTable dt = new DataTable();
        string cmd = "SELECT Sessions AS item ,SUM(CAST(Milkkg AS DECIMAL(18,2))) AS value FROM lp  Where Prdate = '" + d1.ToString().Trim() + "'  AND sessions='AM' GROUP BY Sessions";
    ...
    private double TodayDayMilk()
    {
        ...
        d2 = frmdat.ToString("MM/dd/yyyy");
        double Rval = 0.0;
        string str = "SELECT SUM(CAST(Milkkg AS DECIMAL(18,2))) AS value FROM lp  Where Prdate ='" + d1.ToString().Trim() + "'  AND sessions='AM' GROUP BY Sessions";
    ...
    public void GrandTotal()
    {
        ...
        try
        {
            ...
            //GTotalMilk for Cow
            string strc = "Select  CONVERT(DECIMAL(10,2),sum(milkkg)) as milkkg,CONVERT(DECIMAL(10,2),AVG(fat)) as fat, CONVERT(DECIMAL(10,2),AVG(snf)) as snf  from lp  where prdate='" + date + "' and  sessions='" + sess + "' AND PlantCode IN (157,165,166,167,168) AND fat>0 AND snf>0  group by  sessions";
            ...
            //GTotalMilk
            string str = "Select  CONVERT(DECIMAL(10,2),sum(milkkg)) as milkkg,CONVERT(DECIMAL(10,2),AVG(fat)) as fat, CONVERT(DECIMAL(10,2),AVG(snf)) as snf  from lp  where prdate='" + date + "' and  sessions='" + sess + "' AND fat>0 AND snf>0 group by  sessions";
            SqlCommand cmd = new SqlCommand(str, con);
    ...
    protected void grdLivepro_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (spilval == "157" || spilval == "165" || spilval == "166" || spilval == "167" || spilval == "168")
                {
