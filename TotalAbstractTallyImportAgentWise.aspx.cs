...
public partial class TotalAbstractTallyImportAgentWise : System.Web.UI.Page
{
    ...
    public void getagentlist()
    {
            ...
            if ((pcode == "157") || (pcode == "165") || (pcode == "166") || (pcode == "167") || (pcode == "168") || (pcode == "169"))
            {
              //  str = "select VchDate,(Agent_id + '-'+ Agent_Name + '-' + Plant_Name) as Suppliername,VchDate as SupDate,Plant_Name as Purchaseledgerofmilk,Plant_Name as Itemname,BilledQTy,Rate,Amount,procurementcortageDR,otherincome,procurementMaterialDebtocsp,cateFeedSalesAgentscsp,Canslidsstock,billadvancekbp,Sfatkg,SSnfkg     from (Select *  from (sELECT convert(varchar,Agent_id) as Agent_id,convert(varchar,Frm_date,101) as Frm_date,convert(varchar,To_date,101) as To_date ,REPLACE(CONVERT(VARCHAR(9), To_date, 6), ' ', '-') AS [VchDate],Route_id,REPLACE(CONVERT(VARCHAR(9), To_date, 6), ' ', '-') AS [Vch Date],Plant_code,SUM(Smkg) as BilledQTy,Convert(decimal(18,2),(SUM(SAmt) +  SUM(TotAdditions)) / SUM(Smltr)) as Rate,(SUM(SAmt) + SUM(TotAdditions)) as Amount,SUM(Scaramt) as procurementcortageDR,SUM(Roundoff) as otherincome,SUM(Ai) as procurementMaterialDebtocsp,SUM(Feed) AS cateFeedSalesAgentscsp,SUM(can) as Canslidsstock,SUM(Billadv) as billadvancekbp,SUM(Sfatkg) as Sfatkg,SUM(SSnfkg) as SSnfkg,plant_code as payplantcode FROM  Paymentdata    WHERE Plant_code='" + pcode + "'  AND   Frm_date='" + FDATE + "' AND  To_date='" + TODATE + "'  group by   Agent_id,Route_id,plant_code,Frm_date,To_date,Plant_code) as paydata left join (Select Agent_Id as Amagent,convert(varchar,Agent_Name) as  Agent_Name  from Agent_Master   where Plant_code='" + ddl_Plantname.SelectedItem.Value + "'   group by Agent_Id,Agent_Name) as am on paydata.Agent_id=am.Amagent) as ll left join (Select  plant_code as pmpcode,convert(varchar,Plant_Name) as Plant_Name    from Plant_Master where plant_code='" + ddl_Plantname.SelectedItem.Value + "'  group by plant_code,Plant_Name) as pm on ll.Plant_code = pm.pmpcode";
                str = "select VchDate,(Agent_id + '-'+ Agent_Name + '-' + Plant_Name) as Suppliername,VchDate as SupDate,Plant_Name as Purchaseledgerofmilk,Plant_Name as Itemname,BilledQTy,Rate,Amount,procurementcortageDR,otherincome,procurementMaterialDebtocsp,cateFeedSalesAgentscsp,Canslidsstock,billadvancekbp,Sfatkg,SSnfkg, loan, otherdeduction from (Select *  from (sELECT convert(varchar,Agent_id) as Agent_id,convert(varchar,Frm_date,101) as Frm_date,convert(varchar,To_date,101) as To_date ,REPLACE(CONVERT(VARCHAR(9), To_date, 6), ' ', '-') AS [VchDate],Route_id,REPLACE(CONVERT(VARCHAR(9), To_date, 6), ' ', '-') AS [Vch Date],Plant_code,SUM(Smkg) as BilledQTy,Convert(decimal(18,2),(SUM(SAmt) +  SUM(TotAdditions)) / SUM(Smltr)) as Rate,(SUM(SAmt) + SUM(TotAdditions)) as Amount,SUM(Scaramt) as procurementcortageDR,SUM(Roundoff) as otherincome,SUM(Ai) as procurementMaterialDebtocsp,SUM(Feed) AS cateFeedSalesAgentscsp,SUM(can) as Canslidsstock,SUM(Billadv) as billadvancekbp,SUM(Sfatkg) as Sfatkg,SUM(SSnfkg) as SSnfkg, plant_code as payplantcode, SUM(Sinstamt) AS loan, SUM(others) AS otherdeduction FROM  Paymentdata    WHERE Plant_code='" + pcode + "'  AND   Frm_date='" + FDATE + "' AND  To_date='" + TODATE + "'  group by   Agent_id,Route_id,plant_code,Frm_date,To_date,Plant_code) as paydata left join (Select Agent_Id as Amagent,convert(varchar,Agent_Name) as  Agent_Name  from Paymentdata   where Plant_code='" + ddl_Plantname.SelectedItem.Value + "' and  Frm_date='" + FDATE + "'  AND To_date= '" + TODATE + "'  group by Agent_Id,Agent_Name) as am on paydata.Agent_id=am.Amagent) as ll left join (Select  plant_code as pmpcode,convert(varchar,Plant_Name) as Plant_Name    from Plant_Master where plant_code='" + ddl_Plantname.SelectedItem.Value + "'  group by plant_code,Plant_Name) as pm on ll.Plant_code = pm.pmpcode";
            }
            else
            { 
                //cow
             //   str = "select VchDate,(Agent_id + '-'+ Agent_Name + '-' + Plant_Name) as Suppliername,VchDate as SupDate,Plant_Name as Purchaseledgerofmilk,Plant_Name as Itemname,BilledQTy,Rate,Amount,procurementcortageDR,otherincome,procurementMaterialDebtocsp,cateFeedSalesAgentscsp,Canslidsstock,billadvancekbp,Sfatkg,SSnfkg     from (Select *  from (sELECT convert(varchar,Agent_id) as Agent_id,convert(varchar,Frm_date,101) as Frm_date,convert(varchar,To_date,101) as To_date ,REPLACE(CONVERT(VARCHAR(9), To_date, 6), ' ', '-') AS [VchDate],Route_id,REPLACE(CONVERT(VARCHAR(9), To_date, 6), ' ', '-') AS [Vch Date],Plant_code,SUM(Smltr) as BilledQTy,Convert(decimal(18,2),(SUM(SAmt) +  SUM(TotAdditions)) / SUM(Smltr)) as Rate,(SUM(SAmt) + SUM(TotAdditions)) as Amount,SUM(Scaramt) as procurementcortageDR,SUM(Roundoff) as otherincome,SUM(Ai) as procurementMaterialDebtocsp,SUM(Feed) AS cateFeedSalesAgentscsp,SUM(can) as Canslidsstock,SUM(Recovery) as billadvancekbp,SUM(Sfatkg) as Sfatkg,SUM(SSnfkg) as SSnfkg,plant_code as payplantcode FROM  Paymentdata    WHERE Plant_code='" + pcode + "'  AND   Frm_date='" + FDATE + "' AND  To_date='" + TODATE + "'  group by   Agent_id,Route_id,plant_code,Frm_date,To_date,Plant_code) as paydata left join (Select Agent_Id as Amagent,convert(varchar,Agent_Name) as  Agent_Name  from Agent_Master   where Plant_code='" + ddl_Plantname.SelectedItem.Value + "'   group by Agent_Id,Agent_Name) as am on paydata.Agent_id=am.Amagent) as ll left join (Select  plant_code as pmpcode,convert(varchar,Plant_Name) as Plant_Name    from Plant_Master where plant_code='" + ddl_Plantname.SelectedItem.Value + "'  group by plant_code,Plant_Name) as pm on ll.Plant_code = pm.pmpcode";
                ...
                str = "select VchDate,(Agent_id + '-'+ Agent_Name + '-' + Plant_Name) as Suppliername,VchDate as SupDate,Plant_Name as Purchaseledgerofmilk,Plant_Name as Itemname,BilledQTy,Rate,Amount,procurementcortageDR,otherincome,procurementMaterialDebtocsp,cateFeedSalesAgentscsp,Canslidsstock,billadvancekbp,Sfatkg,SSnfkg, loan, otherdeduction    from (Select *  from (sELECT convert(varchar,Agent_id) as Agent_id,convert(varchar,Frm_date,101) as Frm_date,convert(varchar,To_date,101) as To_date ,REPLACE(CONVERT(VARCHAR(9), To_date, 6), ' ', '-') AS [VchDate],Route_id,REPLACE(CONVERT(VARCHAR(9), To_date, 6), ' ', '-') AS [Vch Date],Plant_code,SUM(Smltr) as BilledQTy,Convert(decimal(18,2),(SUM(SAmt) +  SUM(TotAdditions)) / SUM(Smltr)) as Rate,(SUM(SAmt) + SUM(TotAdditions)) as Amount,SUM(Scaramt) as procurementcortageDR,SUM(Roundoff) as otherincome,SUM(Ai) as procurementMaterialDebtocsp,SUM(Feed) AS cateFeedSalesAgentscsp,SUM(can) as Canslidsstock,SUM(Recovery) as billadvancekbp,SUM(Sfatkg) as Sfatkg,SUM(SSnfkg) as SSnfkg,plant_code as payplantcode,SUM(Sinstamt) AS loan, SUM(others) AS otherdeduction FROM  Paymentdata    WHERE Plant_code='" + pcode + "'  AND   Frm_date='" + FDATE + "' AND  To_date='" + TODATE + "'  group by   Agent_id,Route_id,plant_code,Frm_date,To_date,Plant_code) as paydata left join (Select Agent_Id as Amagent,convert(varchar,Agent_Name) as  Agent_Name  from Paymentdata   where Plant_code='" + ddl_Plantname.SelectedItem.Value + "' and  Frm_date='" + FDATE + "'  AND To_date= '" + TODATE + "'  group by Agent_Id,Agent_Name) as am on paydata.Agent_id=am.Amagent) as ll left join (Select  plant_code as pmpcode,convert(varchar,Plant_Name) as Plant_Name    from Plant_Master where plant_code='" + ddl_Plantname.SelectedItem.Value + "'  group by plant_code,Plant_Name) as pm on ll.Plant_code = pm.pmpcode";
            }
            ...
            if (Checkstatus != 1)
            {
                ...
                foreach (DataRow tally in tallyagent.Rows)
                {
                    ...
                    double Otherdeduction = Convert.ToDouble(tally[17]);
                    ...
                    string ItemName;
                    if ((pcode == "155") || (pcode == "156") || (pcode == "158") || (pcode == "159") || (pcode == "161") || (pcode == "162") || (pcode == "163") || (pcode == "164") || (pcode == "173") || (pcode == "174"))
                    {
                        ItemName = "COW MILK";
                    ...
                    if (ddl_Plantname.SelectedItem.Text == "ALAPATTI")
                    {
                        Tallyno = "ALP";
                    }
                    ...
                    {
                        GETINSERT = "INSERT INTO TallypurchaseImportAgentWsie(VchNo,TallyVchNo,VchDate,SupplierinvNo,SupplierinvDate,SupplierName,PurchaseLedger,ItemName,Billedqty,Rate,Amount,Cartage,otherincome,Material,Feed,CanorStock,BillAdvance,UserName,Datetime,plant_code,AvgFat,Avgsnf,Kgfat,KgSnf,Kgrate,Ts,Rts,FrmDate,Todate,loandedamt,otherdedamt)  values ('" + ViewState["maxtid"] + "','" + gettp + "','" + vchDate + "','" + Supplierinvno + "','" + Supplierdate + "','" + Suppliername + "','" + Ledgerofpurcharseplant + "','" + ItemName + "','" + Billedqty + "','" + Rate + "','" + Amount + "','" + Cartage + "','" + Others + "','" + material + "','" + Feed + "','" + Canorstock + "','" + BillAdvance + "','" + Session["Name"].ToString() + "','" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','" + pcode + "','" + avgfat1 + "','" + avgSnf1 + "','" + fatkg1 + "','" + snfkg1 + "','0','" + ts1 + "','0','" + FDATE + "','" + TODATE + "', '" + loan + "', '" + Otherdeduction + "');";
                        con = DB.GetConnection();
                        SqlCommand COMM = new SqlCommand(GETINSERT, con);
                        COMM.ExecuteNonQuery();
                    }
    ...
    public void GETTALLYREPORT()
    {
        ...
        {
            con = DB.GetConnection();
            string GETSTR = "SELECT TallyVchNo ,REPLACE(CONVERT(VARCHAR(9), VchDate, 6), ' ', '-') AS [VchDate],SupplierinvNo,SupplierinvDate,SupplierName,PurchaseLedger,ItemName,Billedqty,Rate,Amount,Cartage,otherincome,Material,Feed,CanorStock,BillAdvance,AvgFat,Avgsnf,Ts,Rts,Kgfat,KgSnf,Kgrate,otherdedamt FROM TallypurchaseImportAgentWsie WHERE PLANT_CODE='" + pcode + "' AND  FrmDate='" + FDATE + "' and Todate='" + TODATE + "'  ORDER BY VchNo asc   ";
            ...
            if ((pcode == "155") || (pcode == "156") || (pcode == "158") || (pcode == "159") || (pcode == "161") || (pcode == "162") || (pcode == "163") || (pcode == "164") || (pcode == "173"))
            {
            ...
            //    string SupplierName = drtally[4].ToString() + "Route Milk Bills" + ddl_Plantname.SelectedItem.Text;
                ...
                if ((pcode == "155") || (pcode == "156") || (pcode == "158") || (pcode == "159") || (pcode == "161") || (pcode == "162") || (pcode == "163") || (pcode == "164") || (pcode == "171") || (pcode == "172") || (pcode == "173"))
                {
                    ItemName = "COW MILK";
                }
                else
                ...
                double otherdeduction = Convert.ToDouble(drtally[23]);
                ...
                double minusamount = Convert.ToDouble(OtherIncomeE + ProcurementMaterialDebtorcspP + CatteleFeedSalesAgentscspP + CansLidsStockK + BillAdvanceE + otherdeduction);
    ...
    public void GETTALLYREPORT1()
    {
        try
        {
            ...
            string GETSTR = "SELECT TallyVchNo ,REPLACE(CONVERT(VARCHAR(9), VchDate, 6), ' ', '-') AS [VchDate],SupplierinvNo,SupplierinvDate,SupplierName,PurchaseLedger,ItemName,Billedqty,Rate,Amount,Cartage,otherincome,Material,Feed,CanorStock,BillAdvance,AvgFat,Avgsnf,Ts,Rts,Kgfat,KgSnf,Kgrate, loandedamt,otherdedamt   FROM TallypurchaseImportAgentWsie WHERE PLANT_CODE='" + pcode + "' AND  FrmDate='" + ViewState["FD"] + "' and Todate='" + ViewState["TTODATE"] + "'  ORDER BY VchNo asc   ";
            SqlCommand cmd3 = new SqlCommand(GETSTR, con);
            SqlDataAdapter DA3 = new SqlDataAdapter(cmd3);
            ...
            foreach (DataRow drtally in TALLREPORT.Rows)
            {
             ...
             //   string SupplierName = drtally[4].ToString() + "Route Milk Bills" + ddl_Plantname.SelectedItem.Text;
               ...
               // string conamount = (Amount).ToString("f2");
                ...
                string other = drtally[24].ToString();
                if (other != "")
                {
                }
                else
                ...
                double otherdeduction = Convert.ToDouble(other);
