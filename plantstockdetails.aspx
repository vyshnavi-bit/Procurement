<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="plantstockdetails.aspx.cs" Inherits="plantstockdetails" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function get_plantwiseclosing_details() {
            var date = document.getElementById("<%=txt_FromDate.ClientID %>").value;
            var data = { 'operation': 'get_plantwiseclosing_details', 'date': date};
            var s = function (msg) {
                if (msg) {
                    if (msg.length > 0) {
                        createChart(msg);
        ...
        function createChart(databind) {
            ...
            for (var k = 0; k < databind.length; k++) {
                var month = databind[k].Date;
                var qty = databind[k].quantity;
                var Status = databind[k].status;
                newXarray = month.split(',');
                for (var i = 0; i < qty.length; i++) {
                    newYarray.push({ 'data': qty[i].split(','), 'name': Status[i] });
                }
            }
    ...
    <table align="center" width="50%" style="border: thin ridge #008080">
        <tr>
            <td class="style2" style="text-align: center;">
                From<asp:TextBox ID="txt_FromDate" runat="server" class="text1" Width="175px"></asp:TextBox>
                <asp:CalendarExtender ID="txt_FromDate_CalendarExtender" runat="server" TargetControlID="txt_FromDate"
        ...
        <tr align="center">
            <td align="CENTER" style="width: 55%; text-align: center">
               <input type="button" value="Genarate" onclick="genaratechart()" />
            </td>
        </tr>
    </table>
    <br />
