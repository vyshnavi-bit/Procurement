...
public partial class rmksccwiseapprove : System.Web.UI.Page
{
    ...
    private void BindprocurementRemarksData()
    {
        try
        {
            ...
            ds = Bllproimp.LoadProocurementRemarksDataCCWISEAPPROVEs(ccode, pcode, d1, ses, IncdecFlag);
            if (ds != null)
            {
        ...
        try
        {
            ...
            if (count > 0)
            {
                ...
                for (int i = 0; i <= CheckBoxList1.Items.Count - 1; i++)
                {
                    ...
                    if (Convert.ToDouble(modifyfat) > Convert.ToDouble(fat))
                    {
                        double acfat = Convert.ToDouble(fat);
                        double mafat = acfat + 0.1;
                        modifyfat = mafat.ToString();
                    }
                    custDT.Rows.Add(dr);
