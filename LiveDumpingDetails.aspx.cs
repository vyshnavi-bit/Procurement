...
public partial class LiveDumpingDetails : System.Web.UI.Page
{
    ...
    private void RoutewiseLineGraph(string Rid, string pcode)
    {
        try
        {
            ...
            string query = string.Format("SELECT (REPLACE( Convert(Nvarchar(5),EndingTime), ':', '.' ))AS StartingTime,REPLACE( Convert(Nvarchar(5),LabCaptureTime), ':', '.' )AS LabCaptureTime ,sampleno FROM Lp Where PlantCode='" + pcode + "' AND Prdate='" + d1.ToString().Trim() + "' AND RouteId IS NOT NULL AND RouteId='" + Rid + "'   AND sessions='" + sess + "' order by sampleno"); 
