...
using System.Data;
public partial class compressioReport : System.Web.UI.Page
{
    SqlCommand cmd;
    string Branchid = "";
    SalesDBManager vdm;
    ...
    protected void btn_Generate_Click(object sender, EventArgs e)
    {
        try
        {
          ...
          //convert(varchar, doe, 103) as date ,
            cmd.Parameters.Add("@d1", GetLowDate(fromdate));
            cmd.Parameters.Add("@d2", GetHighDate(todate));
            DataTable dtcompresssionrunninghours = SalesDB.SelectQuery(cmd).Tables[0];
            cmd = new SqlCommand("SELECT date,session as session ,ontime,offtime,totalliters FROM milkrechilling where  Plant_code=@plantcode  and  date between @d1 and @d2 ");
            cmd.Parameters.Add("@plantcode", plantcode);
            cmd.Parameters.Add("@d1", GetLowDate(fromdate));
            cmd.Parameters.Add("@d2", GetHighDate(todate));
            DataTable dtmilkrechilling = SalesDB.SelectQuery(cmd).Tables[0];
            //convert(varchar, doe, 103) as date,
            ...
            if (dtcompresssionrunninghours.Rows.Count > 0)
            {
                ...
                grdmilkrecheller.DataSource = dtmilkrechilling;
                grdmilkrecheller.DataBind();
                hidepanel.Visible = true;
            }
            else
            {
